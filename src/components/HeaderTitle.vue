<template>
  <nav class="bg-gray-800">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="flex flex-col md:flex-row py-3 items-center justify-between">
        <div>
          <a href="https://www.linkedin.com/in/ines-garcia-zapata-0b64b3175/"
            class="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium">LinkedIn</a>
          <a href="https://github.com/PetraZeta"
            class="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium">GitHub</a>
        </div>
        <div class="flex-col text-gray-300">
          <h2 class="text-2xl mx-auto text-center text-gray-500 font-bold">
            Lista de usuarios aleatorios
          </h2>
          <h1 class="text-l text-center bold">Prueba técnica Ninja Talent</h1>
        </div>

        <div class="flex items-center space-x-4">
          <div class="p-2">
            <p class="text-gray-300 text-xs">Inés García Zapata</p>
            <p class="text-gray-500 text-xs text-center">Frontend developer</p>
          </div>
          <div class="flex-shrink-0">
            <img class="h-8 w-8 rounded-full"
              src="https://media.licdn.com/dms/image/D4D03AQEonDURis3Yyg/profile-displayphoto-shrink_400_400/0/1710493624572?e=1718236800&v=beta&t=8eWhiE5aCVkjU-fuZh1QDVgs3zrge--hIhM0b8ndc3I"
              alt="Inés García Zapata" />
          </div>
        </div>
      </div>
    </div>
  </nav>
</template>
